        ...
        void UpdateVisibility()
        {
            ...
            foreach( var c in colliders)
            {
                ...
                if (go != null)
                {
                    ...
                    float angle = Vector3.Angle(transform.forward, go.transform.position - transform.position);

                    bool isInClosedField = Vector3.Distance(go.transform.position, transform.position) <= closeFieldDistance;
